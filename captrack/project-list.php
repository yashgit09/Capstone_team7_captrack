...
if(isset($_SESSION['id']))
{
	...
	{
		$id2=$_POST['id'];
		$sql2 = $conn->prepare("UPDATE projects set status='OnHold' where id='".$id2."' AND project_added_by='".$_SESSION['id']."' ");
		$sql2->execute();  
	...
	// {
	// 	$id3=$_POST['id'];
	// 	$sql3 = $conn->prepare("UPDATE projects set status='Completed' where id='".$id3."' AND project_added_by='".$_SESSION['id']."' "); 
	// 	$sql3->execute();  
	// 	$_SESSION['leaves1']="<div class='alert alert-success'><center>Status Update Successfully.</center></div>";
	// }
	...
	if(isset($_POST['Completed']))
	{
		...
		$sql = $conn->prepare("SELECT * FROM projects where id='".$id3."' AND project_added_by='".$_SESSION['id']."' "); 
		$sql->execute();
		$result = $sql->fetch(PDO::FETCH_OBJ);
		$deadline=$result->deadline;
//echo '<script type="text/javascript">alert("'.$result->status.'");</script>';

		if($result->status=='Approval_req')
		{
			if($deadline>=$present_date)
			{
				$sql31 = $conn->prepare("UPDATE registration set points=points+100 where id='".$result->employee_id."' "); 
				$sql31->execute();  
	//echo '<script>alert("Welcome to Geeks for Geeks")</script>';
				$sql3 = $conn->prepare("UPDATE projects set status='Completed' where id='".$id3."'  "); 
				$sql3->execute(); 
				$_SESSION['leaves1']="<div class='alert alert-success'><center>Status Update Successfully.</center></div>";
			}
		}
	...
	{
		$id3=$_POST['id'];
		$sql3 = $conn->prepare("UPDATE projects set status='Active' where id='".$id3."' AND project_added_by='".$_SESSION['id']."' "); 
		$sql3->execute();  
		$_SESSION['leaves1']="<div class='alert alert-success'><center>Status Update Successfully.</center></div>";
	}
	?>
							...
							<div class="col-xl-12 col-md-12 col-lg-12">
								<div class="card">
									<div class="card-header  border-0">
										<h4 class="card-title">Project Added By Me</h4>
									</div>
										...
										<div class="table-responsive recent_jobs p-0 card-body">
											<table class="table mg-b-0 text-nowrap">
												<tbody>
													<?php
													$i = 1;
													$sql = $conn->prepare("SELECT * FROM projects where project_added_by='".$_SESSION['id']."'");
													$sql->execute();
													...
													foreach($result as $data)
													{
														...
														<tr class="border-bottom">
															...
															<?php } if($data->status=='Approval_req') { ?>
																<td class="text-left"><span class="task-btn border-orange text-orange">Approval Requested</span></td>
															...
															<td class="text-left d-flex">
																...
																<?php if($data->status=='Active'){ ?>
																	<form method="post">
																		...
																		<button name="OnHold" class="action-btns1" type="submit" value="OnHold" title="Put on Hold"><i class="feather feather-stop-circle warning text-warning"></i></button>
																	</form>
																	...
																	<form method="post">
																		<input type="hidden" name="id" value="<?php echo $data->id; ?>">
																		<button name="Completed" class="action-btns1" type="submit" value="Completed" onClick="return confirm('Are you sure?')" title="Move to Complete"><i class="feather feather-send success text-success"></i></button>
																	</form>
																<?php } elseif ($data->status=='OnHold') { ?>
																	<form method="post">
																		<input type="hidden" name="id" value="<?php echo $data->id; ?>">
																		<button name="Active" class="action-btns1" type="submit" value="Active" title="Active"><i class="feather feather-stop-circle success text-success"></i></button>
																	</form>
																...
																<?php } elseif ($data->status=='Approval_req') { ?>
